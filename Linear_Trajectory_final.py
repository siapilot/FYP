...
def main():
    ...
    wp = (5,5,0)  # Waypoints (x, y, z), for the sake to enter offboard. wp defined later
    speed = (0.1,0.1,0.1)
    ...
    while (not rospy.is_shutdown()):
        ...
        show_pos = True                             #Change to False to not show position info
        ...
        if (t_elapsed <= 20):
            raw = create_msg(0,0,2,0,0,1)

        elif (abs(wp[0] - curr_pose.pose.position.x) >= wp_i[0]) and (abs(wp[1] - curr_pose.pose.position.y) >= wp_i[1]):
            raw = track(wp[0], wp[1], 2, Vg_x*1.5, Vg_y*1.5)        #Drone target height z = 2
            print(1)

        elif (abs(wp[0] - curr_pose.pose.position.x) <= 0.4) and (abs(wp[1] - curr_pose.pose.position.y) <= 0.4) and (abs(wp[2] - curr_pose.pose.position.z) <= 0.5):
            raw = hovering(wp[0], wp[1], 0.05, Vg_x*1.05, Vg_y*1.05, -0.1)       #Drone Vz = 0
            if kill_flag == False:
                local_raw_pub.publish(raw)
            print(2)

            if (abs(wp[0] - curr_pose.pose.position.x) <= 0.1) and (abs(wp[1] - curr_pose.pose.position.y) <= 0.1) and abs(wp[2] - curr_pose.pose.position.z) <= 0.08:
                service.kill()
                final_pose = False
                service.land()
            ...
            raw = descend(wp[0], wp[1], 0.4, Vg_x*1.15, Vg_y*1.15)        
            print(4)
        ...
        elif (abs(wp[0] - curr_pose.pose.position.x) < 5) and (abs(wp[1] - curr_pose.pose.position.y) < 5):
            raw = slowdown(wp[0], wp[1], 2, Vg_x*1.3, Vg_y*1.3, 0.0)          #Drone Vz = 0.0
            print(6)

        elif (abs(wp[0] - curr_pose.pose.position.x) < 8) and (abs(wp[1] - curr_pose.pose.position.y) < 8):
            raw = slowdown(wp[0], wp[1], 2, Vg_x*1.4, Vg_y*1.4, 0.0)          #Drone Vz = 0.0
            print(7)
